...
params = {
    ...
    'a': 3.801113,       # Semi-major axis in stellar radii
    'Rs': 1.437300832999673,       # Stellar radius in solar radii
    'Rp': 0.172615,                   # Planet radius in solar radii
    'inc': 88.443885,       # Orbital inclination in degrees
    'c0': 0.994494,      # Polynomial order 0 coefficient
    'c1': -0.000701,                     # Polynomial order 1 coefficient
    'r0': 0.001742,   # Ramp magnitude
    'r1': 0.014829                      # Ramp timescale in days
}
...
with model:
    ...
    if FIT_ORBITAL:
        ...
        labels = ['planet_amp', 'Rp/R*', 'a/R*', 'inc', 'Y10', 'Y11c', 'Y11s', 'Y20', 'Y21c', 'Y21s', 'Y22c', 'Y22s',
        ...
        labels = ['planet_amp', 'Y10', 'Y11c', 'Y11s', 'Y20', 'Y21c', 'Y21s', 'Y22c', 'Y22s',
                  'c0', 'c1', 'r0', 'r1', 'error_inflation']
...
plt.savefig('wasp121b_fit_map_orbital.png')
plt.close()
...
plt.tight_layout()
plt.savefig('wasp121b_maps_orbital.png')
plt.close()
...
plt.tight_layout()
plt.savefig('wasp121b_map_profiles_orbital.png')
plt.close()

# Save the fitted parameters
with open('wasp121b_map_orbital_params.txt', 'w') as f:
...
plt.xlabel("Time [days from start]", fontsize=12)
plt.ylabel("Relative Flux", fontsize=12)
plt.title("WASP-121b Eclipse Light Curve with Phase Curve Model")

plt.savefig('wasp121b_fit_pc.png')
plt.close()
...
with open('wasp121b_flux_curves.txt', 'w') as f:
...
plt.tight_layout()
plt.savefig('wasp121b_summary.png')
plt.close() 